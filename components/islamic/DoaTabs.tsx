...
export function DoaTabs({ sughra, kubra }: DoaTabsProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-16 w-full flex flex-col gap-6">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-md border border-gray-100 -mt-10 relative z-10 flex flex-col lg:flex-row items-center justify-between gap-4">
        {/* Selector 1: Versi (Sughra vs Kubra) */}
        <div className="flex flex-col gap-1.5 w-full lg:flex-1">
          <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider px-1 flex items-center gap-1">
            <Layers className="w-3 h-3 text-forest-600" />
            Pilih Versi
          </span>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`flex-1 px-3 py-2 rounded-xl text-xs font-bold transition-all shadow-sm border flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 ${
                showGlobalLatin
                  ? "bg-forest-600 text-white border-forest-600 hover:bg-forest-750"
                  : "bg-gray-100 text-gray-500 border-transparent hover:text-gray-850 hover:bg-gray-200/50"
              }`}
            >
      ...
      <div className="text-center text-xs text-gray-400 font-medium tracking-wide mt-2">
        Membaca <span className="font-bold text-forest-600">Al-Ma'surat {version === "sughra" ? "Sughra" : "Kubra"}</span> untuk waktu <span className="font-bold text-forest-600">{time === "morning" ? "Pagi" : "Sore/Petang"}</span>
      </div>
      ...
      <div className="space-y-5 w-full">
        {activeItems.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
            <p className="text-gray-400 text-sm">Belum ada dzikir untuk kategori ini.</p>
          </div>
        ) : (
